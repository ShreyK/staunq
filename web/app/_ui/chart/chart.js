...
// import Chat from '@/app/_ui/chat/chat';
...
export async function Chart() {
  ...
  const { trades, orderBook, refetchTrades, refetchOrderBook, interval, updateInterval, symbol } = appContext;
  ...
  const startDataFetchingInterval = () => {
    ...
    intervalRef.current = setInterval(() => {
      startTransition(async () => {
        ...
        const tradesResponse = await fetchTrades(symbol, interval)
  ...
  const reloadData = (symbol, chartSeries, currInterval, from, to, refresh) => {
    startTransition(async () => {
      const tradesResponse = await refetchTrades(symbol, currInterval, from, to)
      const newDataSorted = reduceTrades(tradesResponse)
      if (newDataSorted.length === 0) {
        return
      }
      if (refresh) {
        chartSeries.setData(newDataSorted);
        return
      }

      const currData = chartSeries.data()
      ...
      chartSeries.setData(mergedData);
    })
  }
  const onIntervalUpdated = (value) => {
    startTransition(() => {
  ...
  const onVisibleLogicalRangeChange = useCallback(debounce((newVisibleLogicalRange) => {
    if (newVisibleLogicalRange !== null && !!chartInstanceRef.current) {
      ...
      if (barsInfo !== null && barsInfo.barsBefore < 0) {
        ...
        reloadData(symbol, chartSeriesInstanceRef.current, interval, from, to)
      }
    }
  }, 1000), [interval])
